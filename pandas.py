...
sheet = pd.read_excel(r"notas_planilha_modelo.xlsx", skiprows=(0, 2))#pegar o caminho do seu pc
...
def calcular_nota(x):
    ...
    media_cp_ch = round(((notas/4)*0.4), 2) # Média dos cps + sprints do 2o semestre
    media_s2 = round(((6 - media_s1) / 0.6 ), 2) # Nota necessária para o 2o semestre
    nota_necessaria_GS = round(((media_s2 - media_cp_ch) /6), 2) # Nota necessária na GS para ser aprovado no 2o semestre
    ...
    sheet['Média Checkpoint / Challenge'][x] = media_cp_ch

    print('\n\n',sheet['RM'][x], sheet['Nome'][x] , '\nDisciplina: Computacional Thinking Using Python\nSemestre 1: ', sheet['Semestre_1'][x],
     '\n\t\tSemestre 2\nCheckpoints (média): ', media_cp, '\nChallenge: ', media_ch,'\n\nNota mínima na Global Solution para aprovação: ', nota_necessaria_GS,'\n\n')

x = int(input("Digite o índice das notas: (0 por exemplo)"))

calcular_nota(x)
# sheet.to_excel('notas_planilha_modelo.xlsx', index=False)
